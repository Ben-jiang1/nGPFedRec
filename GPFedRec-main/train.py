...
# 拼接数据集文件路径
dataset_dir = os.path.join(current_dir, "data", config['dataset'], "ratings.dat")
if config['dataset'] == "ml-1m":
...
for round in range(config['num_round']):
    ...
    p_train_loss,train_loss = engine.fed_train_a_round(all_train_data, round_id=round)
    # break
   
    sp_train_loss_list.append(list(train_loss.values())[0])
    p_train_loss_list.append(list(p_train_loss.values())[0])
    fitlog.add_loss(list(train_loss.values())[0],name="tr_Loss",step=round)
    fitlog.add_loss(list(p_train_loss.values())[0],name="p_tr_Loss",step=round)

    logging.info('-' * 80)
    logging.info('Testing phase!')
    hit_ratio, ndcg, te_loss = engine.fed_evaluate(test_data)
    test_loss_list.append(te_loss)
    fitlog.add_loss(list(te_loss.values())[0],name="eval_loss",step=round)
...
plt.plot(range(len(sp_train_loss_list)), sp_train_loss_list, marker='o', linestyle='-', color='b')  
plt.title('sp_train_loss Over Rounds')  # 设置图形标题  
plt.xlabel('Round')  # 设置x轴标签  
plt.ylabel('sp_train Loss')  # 设置y轴标签  
plt.grid(True)  # 显示网格  
plt.show()  # 显示图形
