...
class Engine(object):
    ...
    def fed_train_single_batch(self, model_client, batch_data, optimizers, user):
        ...
        optimizer_u.step()
        optimizer_i.step()
        return model_client, loss.item(), p_loss
    ...
    def fed_train_a_round(self, all_train_data, round_id):
        ...
        for user in participants:
            ...
            for epoch in range(self.config['local_epoch']):
                for batch_id, batch in enumerate(user_dataloader):
                    ...
                    model_client, batch_loss,p_batch_loss = self.fed_train_single_batch(model_client, batch, optimizers, user)
                    total_loss += batch_loss
                    P_total_loss += p_batch_loss
                    if batch_id == 0:
                        first_batch_loss = batch_loss
            avg_loss = total_loss / len(user_dataloader)
            p_avg_loss = P_total_loss / len(user_dataloader)
            user_losses[user] = avg_loss
            pre_user_losses[user] = first_batch_loss
            p_user_losses[user] = p_avg_loss
        ...
        return p_user_losses,user_losses
